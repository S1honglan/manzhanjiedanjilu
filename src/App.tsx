...
function App() {
  ...
  const dragState = useRef<{
    ...
    currentY: number
    active: boolean
  }>({ orderId: '', index: -1, el: null, ghost: null, startY: 0, currentY: 0, active: false })
  const longPressTimer = useRef<ReturnType<typeof setTimeout> | null>(null)

  function clearDragState() {
    const ds = dragState.current
    if (ds.el) { ds.el.style.opacity = '1'; ds.el = null }
    ...
    document.querySelectorAll('.order-card.drag-occupied').forEach(c => c.classList.remove('drag-occupied'))
    ds.active = false; ds.orderId = ''; ds.index = -1
    if (longPressTimer.current) { clearTimeout(longPressTimer.current); longPressTimer.current = null }
  }

  /** 交换订单位置 */
  function swapOrders(srcId: string, tgtId: string) {
    if (!activeProject || srcId === tgtId) return
    ...
    dragState.current.index = tgtIdx
  }

  /** 开始拖拽（长按或鼠标按下） */
  function startDrag(el: HTMLElement, orderId: string, index: number, clientY: number) {
    const ds = dragState.current
    ds.active = true; ds.orderId = orderId; ds.index = index; ds.el = el
    ds.startY = clientY; ds.currentY = clientY
    ...
    ghost.style.cssText = `
      position:fixed; z-index:9999; pointer-events:none; left:${rect.left}px; top:${rect.top}px;
      width:${rect.width}px; height:${rect.height}px;
      background:${getComputedStyle(el).background};
      border-radius:10px; box-shadow:0 8px 28px rgba(0,0,0,0.28);
      opacity:0.95; display:flex; align-items:center; justify-content:center;
      font-size:16px; font-weight:700; color:#2c3238; padding:14px 16px;
      box-sizing:border-box; transition:top 0.06s ease-out;
    `
    ...
    // 原卡片位置塌陷
    el.classList.add('drag-occupied')
  }

  /** 移动中 */
  function moveDrag(clientY: number) {
    const ds = dragState.current
    if (!ds.active || !ds.ghost) return
    ds.currentY = clientY
    ds.ghost.style.top = `${clientY - ds.ghost.offsetHeight / 2}px`

    // 用幽灵中心点检测下方卡片
    const gRect = ds.ghost.getBoundingClientRect()
    ...
    const card = target?.closest('.order-card[data-order-id]') as HTMLElement | null
    if (card && card !== ds.el) {
  ...
  function endDrag() {
    const ds = dragState.current
    if (!ds.active) return
    document.querySelectorAll('.order-card.drag-over').forEach(c => {
      const tgtId = (c as HTMLElement).getAttribute('data-order-id')
      if (tgtId && ds.orderId) swapOrders(ds.orderId, tgtId)
    })
    clearDragState()
  }
  ...
  const orderTouchStart = useCallback((e: React.TouchEvent, orderId: string, index: number) => {
    ...
    longPressTimer.current = setTimeout(() => startDrag(el, orderId, index, y), 400)
  }, [])

  const orderTouchMove = useCallback((e: React.TouchEvent) => {
    if (!dragState.current.active) {
      // 移动超过阈值取消长按
      if (longPressTimer.current && Math.abs(e.touches[0].clientY - (dragState.current.startY || 0)) > 10) {
  ...
  const orderTouchEnd = useCallback((e: React.TouchEvent) => {
    if (longPressTimer.current) { clearTimeout(longPressTimer.current); longPressTimer.current = null }
    if (dragState.current.active) { e.preventDefault(); endDrag() }
  }, [])

  // ---- 鼠标事件处理器（桌面端） ----
  const orderMouseDown = useCallback((e: React.MouseEvent, orderId: string, index: number) => {
    e.stopPropagation()
    const el = e.currentTarget as HTMLElement
    longPressTimer.current = setTimeout(() => startDrag(el, orderId, index, e.clientY), 350)
  }, [])

  const orderMouseMove = useCallback((e: React.MouseEvent) => {
    if (!dragState.current.active) {
      if (longPressTimer.current && Math.abs(e.clientY - (dragState.current.startY || 0)) > 8) {
        clearTimeout(longPressTimer.current); longPressTimer.current = null
      }
      return
    }
  ...
  const orderMouseUp = useCallback((e: React.MouseEvent) => {
    if (longPressTimer.current) { clearTimeout(longPressTimer.current); longPressTimer.current = null }
    if (dragState.current.active) { e.preventDefault(); endDrag() }
  }, [])
              ...
              {getFilteredOrders().map((order, idx) => {
                return (
                  <div
                    ...
                    onTouchMove={orderTouchMove}
                    onTouchCancel={orderTouchEnd}
                    onMouseDown={e => orderMouseDown(e, order.id, idx)}
                    onMouseMove={orderMouseMove}
                    onMouseUp={orderMouseUp}
                    onMouseLeave={orderMouseUp}
                  >
